...
function App() {
  ...
  const headers: TableProps['headers'] = Object.keys(data[0]).map((columnName) => ({
    key: columnName,
    label: columnName,
  }));
  ...
  return (
    <ChakraBaseProvider theme={theme}>
      <Box alignItems="center" display="flex" flexDir="column" style={{ padding: '20px', height: '500px' }}>
        <Table expandableColumn="Fun Facts" headers={headers} rows={rows} style={{width: '120rem'}} />
      </Box>
    </ChakraBaseProvider>
  );
}
